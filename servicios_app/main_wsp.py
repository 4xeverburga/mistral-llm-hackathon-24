...
async def receive_message(request: Request):
    try:
        ...
        message, from_number = await twilio_wsp.process_incoming_message(form_data)

        if message and from_number:
            # Pasamos el número de teléfono a la función get_completion
            ai_response = mistral_email.get_completion(message, from_number)
            twilio_wsp.send_message(from_number, ai_response)
